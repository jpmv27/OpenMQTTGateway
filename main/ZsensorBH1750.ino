...
void setupZsensorBH1750() {
  Logger.notice(0, F("Setup BH1750 on adress: %H" CR), BH1750_i2c_addr);
  Wire.begin();
...
void MeasureLightIntensity() {
  if (millis() > (timebh1750 + TimeBetweenReadingBH1750)) { //retrieving value of Lux, FtCd and Wattsm2 from BH1750
    Logger.debug(0, F("Creating BH1750 buffer" CR));
    ...
    if (Wire.available() != 2) {
      Logger.error(0, F("Failed to read from LightSensor BH1750!" CR));
    } else {
      i = Wire.read();
      i <<= 8;
      i |= Wire.read();
      ...
      if (Lux != persistedll || bh1750_always) {
        ...
        Logger.debug(0, F("Same lux don't send it" CR));
      }
      ...
      if (ftcd != persistedlf || bh1750_always) {
        ...
        Logger.debug(0, F("Same ftcd don't send it" CR));
      }
      ...
      if (Wattsm2 != persistedlw || bh1750_always) {
        ...
        Logger.debug(0, F("Same wattsm2 don't send it" CR));
      }
