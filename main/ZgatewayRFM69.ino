...
void eeprom_setup() {
  ...
  if (gc_checksum() != pGC->checksum) {
    Logger.debug(OMG_LOGID, F("Factory reset"));
...
void setupRFM69(void) {
  ...
  if (!radio.initialize(pGC->rfmfrequency, pGC->nodeid, pGC->networkid)) {
    Logger.error(OMG_LOGID, F("ZgatewayRFM69 initialization failed"));
  }
  ...
  Logger.notice(OMG_LOGID, F("ZgatewayRFM69 Listening and transmitting at: %d"), freq);
...
bool RFM69toX(void) {
  ...
  if (radio.receiveDone()) {
    ...
    Logger.debug(OMG_LOGID, F("Data received: %s"), (const char*)data);
...
void XtoRFM69(const char* topicOri, const char* datacallback) {
  if (cmpToMainTopic(topicOri, subjectMQTTtoRFM69)) {
    Logger.debug(OMG_LOGID, F("MQTTtoRFM69 data analysis"));
...
void XtoRFM69(const char* topicOri, JsonObject& RFM69data) {
  if (cmpToMainTopic(topicOri, subjectMQTTtoRFM69)) {
    ...
    Logger.debug(OMG_LOGID, F("MQTTtoRFM69 json data analysis"));
    if (data) {
      Logger.debug(OMG_LOGID, F("MQTTtoRFM69 data ok"));
      ...
      if (radio.sendWithRetry(valueRCV, data, strlen(data), 10)) {
        ...
        Logger.error(OMG_LOGID, F("MQTTtoRFM69 sending failed"));
      }
    } else {
      Logger.error(OMG_LOGID, F("MQTTtoRFM69 failed json read"));
    }
  }
}
#  endif
