...
void receivingCommandTask(void* pvParameters) {
  ...
  if (error) {
    ...
    if (jsonBlufi.containsKey("target") && jsonBlufi["target"].is<char*>()) {
      ...
      Logger.notice(OMG_LOGID, F("No target found in the received command using SYS target, default index and save command"));
...
void connection_timeout_callback(void* arg) {
  if (omg_blufi_ble_connected) {
    Logger.notice(OMG_LOGID, F("BluFi connection timeout reached. Disconnecting."));
...
void restart_connection_timer() {
  ...
  if (ret != ESP_OK) {
    Logger.error(OMG_LOGID, F("Failed to start connection timer: %d"), ret);
  }
}
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t* param) {
  ...
  switch (event) {
    case ESP_BLUFI_EVENT_INIT_FINISH:
      Logger.debug(OMG_LOGID, F("BLUFI init finish"));
      esp_blufi_adv_start();
      break;
    case ESP_BLUFI_EVENT_DEINIT_FINISH:
      Logger.debug(OMG_LOGID, F("BLUFI deinit finish"));
    ...
    case ESP_BLUFI_EVENT_BLE_CONNECT:
      Logger.debug(OMG_LOGID, F("BLUFI BLE connect"));
    ...
    case ESP_BLUFI_EVENT_BLE_DISCONNECT:
      Logger.debug(OMG_LOGID, F("BLUFI BLE disconnect"));
      omg_blufi_ble_connected = false;
    ...
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP:
      Logger.debug(OMG_LOGID, F("BLUFI request wifi connect to AP"));
    ...
    case ESP_BLUFI_EVENT_RECV_SLAVE_DISCONNECT_BLE:
      Logger.debug(OMG_LOGID, F("BLUFI recv slave disconnect a ble connection"));
      esp_blufi_disconnect();
      break;
    case ESP_BLUFI_EVENT_RECV_STA_SSID:
      strncpy((char*)gl_sta_ssid, (char*)param->sta_ssid.ssid, param->sta_ssid.ssid_len);
      gl_sta_ssid[param->sta_ssid.ssid_len] = '\0';
      Logger.notice(OMG_LOGID, F("Recv STA SSID %s"), gl_sta_ssid);
      break;
    case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
      ...
      Logger.notice(OMG_LOGID, F("Recv STA PASSWORD"));
      break;
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA: {
      Logger.notice(OMG_LOGID, F("Recv Custom Data %" PRIu32), param->custom_data.data_len);
...
#  endif
    ...
    case ARDUINO_EVENT_WIFI_SCAN_DONE: {
      ...
      if (apCount == 0) {
        Logger.error(OMG_LOGID, F("No AP found"));
        break;
      }
      Logger.debug(OMG_LOGID, F("AP found, count: %d"), apCount);
      esp_blufi_ap_record_t* blufi_ap_list = (esp_blufi_ap_record_t*)malloc(apCount * sizeof(esp_blufi_ap_record_t));
      if (!blufi_ap_list) {
        Logger.error(OMG_LOGID, F("Failed to allocate memory for AP list"));
        break;
      }
      for (int i = 0; i < apCount; ++i) {
        Logger.notice(OMG_LOGID, F("%d: %s, Ch:%d (%ddBm)"), i + 1, WiFi.SSID(i).c_str(), WiFi.channel(i), WiFi.RSSI(i));
      ...
      if (omg_blufi_ble_connected == true) {
        if (esp_blufi_send_wifi_list(apCount, blufi_ap_list) != ESP_OK) {
          Logger.error(OMG_LOGID, F("Failed to send WiFi list"));
        }
      }
      free(blufi_ap_list);
      break;
...
bool startBlufi() {
  ...
  if (ret) {
    Logger.error(OMG_LOGID, F("%s blufi register failed, error code = %x"), __func__, ret);
    return false;
  }
  ...
  char advName[17] = {0};
  // Check length of OMG_GATEWAY_SHORT_NAME
  if (strlen(OMG_GATEWAY_SHORT_NAME) > 3) {
    Logger.error(OMG_LOGID, F("OMG_GATEWAY_SHORT_NAME is too long, max 3 characters"));
    return false;
  }
  snprintf(advName, sizeof(advName), OMG_GATEWAY_SHORT_NAME "_%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  NimBLEDevice::init(advName);
  esp_blufi_gatt_svr_init();
  ble_gatts_start();
  Logger.notice(OMG_LOGID, F("BLUFI started"));
...
bool stopBlufi() {
  ...
  if (result != ESP_OK) {
    Logger.error(OMG_LOGID, F("Failed to deinit blufi profile: %d"), result);
    return false;
  }
  Logger.notice(OMG_LOGID, F("BLUFI stopped"));
  return true;
}
