...
void receivingCommandTask(void* pvParameters) {
  ...
  if (error) {
    ...
    if (jsonBlufi.containsKey("target") && jsonBlufi["target"].is<char*>()) {
      ...
      Logger.notice(0, F("No target found in the received command using SYS target, default index and save command" CR));
...
void connection_timeout_callback(void* arg) {
  if (omg_blufi_ble_connected) {
    Logger.notice(0, F("BluFi connection timeout reached. Disconnecting." CR));
...
void restart_connection_timer() {
  ...
  if (ret != ESP_OK) {
    Logger.error(0, F("Failed to start connection timer: %d" CR), ret);
  }
}
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t* param) {
  ...
  switch (event) {
    case ESP_BLUFI_EVENT_INIT_FINISH:
      Logger.debug(0, F("BLUFI init finish" CR));
      esp_blufi_adv_start();
      break;
    case ESP_BLUFI_EVENT_DEINIT_FINISH:
      Logger.debug(0, F("BLUFI deinit finish" CR));
    ...
    case ESP_BLUFI_EVENT_BLE_CONNECT:
      Logger.debug(0, F("BLUFI BLE connect" CR));
    ...
    case ESP_BLUFI_EVENT_BLE_DISCONNECT:
      Logger.debug(0, F("BLUFI BLE disconnect" CR));
      omg_blufi_ble_connected = false;
    ...
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP:
      Logger.debug(0, F("BLUFI request wifi connect to AP" CR));
      WiFi.begin((char*)gl_sta_ssid, (char*)gl_sta_passwd);
      gl_sta_is_connecting = true;
      break;
    case ESP_BLUFI_EVENT_REQ_DISCONNECT_FROM_AP:
      Logger.debug(0, F("BLUFI request wifi disconnect from AP\n" CR));
      WiFi.disconnect();
      break;
    case ESP_BLUFI_EVENT_REPORT_ERROR:
      Logger.debug(0, F("BLUFI report error, error code %d\n" CR), param->report_error.state);
    ...
    case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
      ...
      Logger.notice(0, F("Recv STA PASSWORD" CR));
      break;
    ...
    case ESP_BLUFI_EVENT_RECV_CUSTOM_DATA: {
      Logger.notice(0, F("Recv Custom Data %" PRIu32 CR), param->custom_data.data_len);
...
#  endif
    ...
    case ARDUINO_EVENT_WIFI_SCAN_DONE: {
      ...
      if (apCount == 0) {
        Logger.error(0, F("No AP found" CR));
        break;
      }
      Logger.debug(0, F("AP found, count: %d" CR), apCount);
      esp_blufi_ap_record_t* blufi_ap_list = (esp_blufi_ap_record_t*)malloc(apCount * sizeof(esp_blufi_ap_record_t));
      if (!blufi_ap_list) {
        Logger.error(0, F("Failed to allocate memory for AP list" CR));
        break;
      }
      for (int i = 0; i < apCount; ++i) {
        Logger.notice(0, F("%d: %s, Ch:%d (%ddBm)" CR), i + 1, WiFi.SSID(i).c_str(), WiFi.channel(i), WiFi.RSSI(i));
      ...
      if (omg_blufi_ble_connected == true) {
        if (esp_blufi_send_wifi_list(apCount, blufi_ap_list) != ESP_OK) {
          Logger.error(0, F("Failed to send WiFi list" CR));
        }
      }
      free(blufi_ap_list);
      break;
...
bool startBlufi() {
  ...
  if (ret) {
    Logger.error(0, F("%s blufi register failed, error code = %x" CR), __func__, ret);
    return false;
  }
  ...
  if (strlen(Gateway_Short_Name) > 3) {
    Logger.error(0, F("Gateway_Short_Name is too long, max 3 characters" CR));
    return false;
  }
  ...
  ble_gatts_start();
  Logger.notice(0, F("BLUFI started" CR));
...
bool stopBlufi() {
  ...
  if (result != ESP_OK) {
    Logger.error(0, F("Failed to deinit blufi profile: %d" CR), result);
    return false;
  }
  Logger.notice(0, F("BLUFI stopped" CR));
  return true;
}
