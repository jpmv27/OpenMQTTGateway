...
void Config_update(JsonObject& data, const char* key, T& var) {
  if (data.containsKey(key)) {
    if (var != data[key].as<T>()) {
      var = data[key].as<T>();
      Logger.notice(0, F("Config %s changed to: %T" CR), key, data[key].as<T>());
    } else {
      Logger.notice(0, F("Config %s unchanged, currently: %T" CR), key, data[key].as<T>());
    }
  }
}
...
bool jsonDispatch(JsonObject& data) {
  ...
  if (data.containsKey("origin") || data.containsKey("topic")) {
    ...
    Logger.error(0, F("No origin or topic in JSON filtered" CR));
...
boolean enqueueJsonObject(const StaticJsonDocument<JSON_MSG_BUFFER>& jsonDoc, int timeout) {
  receivedMessages++;
  if (jsonDoc.size() == 0) {
    Logger.error(0, F("Empty JSON, skipping" CR));
    gatewayState = GatewayState::ERROR;
    return true;
  }
  if (queueLength >= QueueSize) {
    Logger.warning(0, F("%d Doc(s) in queue, doc blocked" CR), queueLength);
    blockedMessages++;
    return false;
  }
  Logger.debug(0, F("Enqueue JSON" CR));
  std::string jsonString;
  serializeJson(jsonDoc, jsonString);
#ifdef ESP32
  // Semaphore check before enqueueing a document
  if (xSemaphoreTake(xQueueMutex, pdMS_TO_TICKS(timeout)) == pdFALSE) {
    Logger.error(0, F("xQueueMutex not taken" CR));
    gatewayState = GatewayState::ERROR;
    blockedMessages++;
    return false;
  }
...
#endif
  Logger.debug(0, F("Queue length: %d" CR), jsonQueue.size());
  return true;
}
...
void buildTopicFromId(JsonObject& Jsondata, const char* origin) {
  if (!Jsondata.containsKey("id")) {
    Logger.error(0, F("No id in Jsondata" CR));
    gatewayState = GatewayState::ERROR;
    return;
  }
  ...
  if (BTConfig.pubBeaconUuidForTopic && !BTConfig.extDecoderEnable && Jsondata.containsKey("model_id") && Jsondata["model_id"].as<std::string>() == "IBEACON") {
    if (Jsondata.containsKey("uuid")) {
      ...
      Logger.error(0, F("No uuid in Jsondata" CR));
      gatewayState = GatewayState::ERROR;
    }
  }
  ...
  Logger.debug(0, F("Origin: %s" CR), Jsondata["origin"].as<const char*>());
}
...
void emptyQueue() {
  ...
  Logger.debug(0, F("Dequeue JSON" CR));
...
#endif
  if (error) {
    Logger.error(0, F("deserialize jsonQueue.front() failed: %s, buffer capacity: %u" CR), error.c_str(), jsonBuffer.capacity());
    gatewayState = GatewayState::ERROR;
  } else {
    if (jsonDispatch(obj))
      queueLengthSum++;
...
bool pub(JsonObject& data) {
  ...
  if (data.size() == 0) {
    Logger.error(0, F("Empty JSON, not published" CR));
    gatewayState = GatewayState::ERROR;
    return res;
  }
  String topic;
  if (data.containsKey("origin") && data["origin"].is<const char*>()) {
    ...
    Logger.error(0, F("No topic or origin in JSON, not published" CR));
    gatewayState = GatewayState::ERROR;
    return res;
  }
...
#if simplePublishing
  Logger.debug(0, F("simplePub - ON" CR));
...
bool pubMQTT(const char* topic, const char* payload, bool retainFlag) {
  ...
  if (SYSConfig.mqtt && !SYSConfig.offline) {
    ...
    if (xSemaphoreTake(xMqttMutex, pdMS_TO_TICKS(QueueSemaphoreTimeOutTask)) == pdFALSE) {
      Logger.error(0, F("xMqttMutex not taken" CR));
      gatewayState = GatewayState::ERROR;
      return res;
    }
#endif
    if (mqtt && mqtt->connected()) {
      Logger.notice(0, F("[ OMG->MQTT ] topic: %s msg: %s " CR), topic, payload);
      res = mqtt->publish(topic, payload, 0, retainFlag);
    } else {
      Logger.warning(0, F("MQTT not connected, aborting the publication" CR));
    }
#ifdef ESP32
    xSemaphoreGive(xMqttMutex);
#endif
  } else {
    Logger.notice(0, F("[ OMG->MQTT deactivated or offline] topic: %s msg: %s " CR), topic, payload);
  }
  return res;
}
...
#  endif
  ...
  preferences.end();
  Logger.notice(0, F("SYS Config_save: %s, result: %d" CR), conf.c_str(), result);
}
...
void SYSConfig_load() {
  ...
  if (preferences.isKey("SYSConfig")) {
    ...
    preferences.end();
    if (error) {
      Logger.error(0, F("SYS config deserialization failed: %s, buffer capacity: %u" CR), error.c_str(), jsonBuffer.capacity());
      gatewayState = GatewayState::ERROR;
      return;
    }
    if (jsonBuffer.isNull()) {
      Logger.warning(0, F("SYS config is null" CR));
      return;
    }
    JsonObject jo = jsonBuffer.as<JsonObject>();
    SYSConfig_fromJson(jo);
    Logger.notice(0, F("SYS config loaded" CR));
  } else {
    preferences.end();
    Logger.notice(0, F("SYS config not found" CR));
  }
}
#else // Function not available for ESP8266
void SYSConfig_load() {}
...
std::pair<String, uint16_t> discoverMQTTbroker() {
  Logger.debug(0, F("Browsing for MQTT service" CR));
  int n = MDNS.queryService("mqtt", "tcp");
  if (n == 0) {
    Logger.warning(0, F("no services found" CR));
  } else {
    Logger.debug(0, F("%d service(s) found" CR), n);
    for (int i = 0; i < n; ++i) {
      Logger.debug(0, F("Service %d %s found" CR), i, MDNS.hostname(i).c_str());
      Logger.debug(0, F("IP %s Port %d" CR), MDNS.IP(i).toString().c_str(), MDNS.port(i));
    }
    if (n == 1) {
      Logger.debug(0, F("One MQTT server found setting parameters" CR));
      return {MDNS.IP(0).toString(), uint16_t(MDNS.port(0))};
    } else {
      Logger.warning(0, F("Several MQTT servers found, please deactivate mDNS and set your default server" CR));
    }
  }
  return {"", 0};
}
...
#if MQTT_BROKER_MODE
void setupMQTT() {
  Logger.notice(0, F("Reconfiguring MQTT broker..." CR));
...
void setupMQTT() {
  Logger.notice(0, F("Reconfiguring MQTT client..." CR));
...
#  if defined(MDNS_SD)
  Logger.debug(0, F("Connecting to MQTT by mDNS without MQTT hostname" CR));
...
#  endif
  ...
  Logger.debug(0, F("Mqtt server: %s" CR), broker_host);
  Logger.debug(0, F("Port: %u" CR), broker_port);
...
#  if AWS_IOT
    {
      ...
      Logger.debug(0, F("MQTT connection count: %d" CR), reconnection_count);
      if (reconnection_count > reconnection_threshold && SYSConfig.mqtt) {
        // Detected instability in MQTT connection
        Logger.warning(0, F("MQTT connection instability detected: %d reconnections in the last %d seconds" CR), reconnection_count, reconnection_window_millis / 1000);
...
#  endif
    ...
    Logger.notice(0, F("Connected to broker" CR));
    ...
    if (cnt_parameters_backup) {
      ...
      Logger.notice(0, F("MQTT connection parameters %d successful" CR), cnt_index);
...
#  endif
  ...
  mqtt->connection_failure_callback = [] {
    ...
    Logger.warning(0, F("failure_number_mqtt: %d" CR), failure_number_mqtt);
...
#  if defined(ESP32)
      Logger.warning(0, F("failed, ssl error code=%d" CR), client->lastError(nullptr, 0));
#  elif defined(ESP8266)
      Logger.warning(0, F("failed, ssl error code=%d" CR), client->getLastSSLError());
#  endif
    }

    if (cnt_parameters_backup) {
      // this was the first attempt to connect to a new server and it failed, revert to old settings
      Logger.error(0, F("MQTT connection failed, reverting to previous settings" CR));
      gatewayState = GatewayState::ERROR;
...
#  ifndef ESPWifiManualSetup
      ...
      for (int i = 0; i < cnt_parameters_array_size; i++) {
        ...
        if (cnt_parameters_array[cnt_index].validConnection) {
          Logger.notice(0, F("Connection %d valid, switching" CR), cnt_index);
          saveConfig();
          break;
        } else {
          Logger.notice(0, F("Connection %d not valid" CR), cnt_index);
        }
      }
#  endif
      ...
      while (
        ...
        Logger.warning(0, F("OTA might be still active (activity %d ms ago)" CR), millis_since_last_ota);
        ArduinoOTA.handle();
        delay(100);
      }
      ESPRestart(1);
...
#  if WifiGMode == true
  if (esp_wifi_set_protocol(WIFI_IF_STA, WIFI_PROTOCOL_11B | WIFI_PROTOCOL_11G) != ESP_OK) {
    Logger.error(0, F("Failed to change WifiMode." CR));
    gatewayState = GatewayState::ERROR;
  }
#  endif
...
#  endif
  ...
  if (err != ESP_OK) {
    Logger.notice(0, F("Could not get protocol!" CR));
  }
  if (getprotocol & WIFI_PROTOCOL_11N) {
    Logger.notice(0, F("WiFi_Protocol_11n" CR));
  }
  if (getprotocol & WIFI_PROTOCOL_11G) {
    Logger.notice(0, F("WiFi_Protocol_11g" CR));
  }
  if (getprotocol & WIFI_PROTOCOL_11B) {
    Logger.notice(0, F("WiFi_Protocol_11b" CR));
  }

#  ifdef WifiPower
  Logger.notice(0, F("Requested WiFi power level: %i" CR), WifiPower);
  WiFi.setTxPower(WifiPower);
#  endif
  Logger.notice(0, F("Operating WiFi power level: %i" CR), WiFi.getTxPower());
}
#endif
...
#  ifdef WifiPower
  Logger.notice(0, F("Requested WiFi power level: %i dBm" CR), WifiPower);
...
#  endif
  ...
  Logger.notice(0, F("OpenMQTTGateway Version: " OMG_VERSION CR));

#ifdef ESP32_EXT0_WAKE_PIN
  Logger.notice(0, F("Setting EXT0 Wakeup for deep sleep." CR));
  ...
  if (esp_sleep_enable_ext0_wakeup(wake_pin0, ESP32_EXT0_WAKE_PIN_STATE) != ESP_OK) {
    Logger.error(0, F("Failed to set deep sleep EXT0 Wakeup." CR));
    gatewayState = GatewayState::ERROR;
  }
#endif
#ifdef ESP32_EXT1_WAKE_PIN
  Logger.notice(0, F("Setting EXT1 Wakeup for deep sleep." CR));
  ...
  if (esp_sleep_enable_ext1_wakeup(wake_pin_bitmask, wake_state1) != ESP_OK) {
    Logger.error(0, F("Failed to set deep sleep EXT1 Wakeup." CR));
    gatewayState = GatewayState::ERROR;
  }
#endif
/*
...
*/
  ...
  if (loadConfigFromFlash()) { // Config present
    Logger.notice(0, F("Config loaded from flash" CR));
#  ifdef ESP32_ETHERNET
...
#  endif
    ...
    Logger.notice(0, F("No config in flash, launching wifi manager" CR));
    // In failSafeMode we don't want to setup wifi manager as it has already been done before
    if (!failSafeMode) setupWiFiManager();
  }

#endif
  Logger.debug(0, F("OpenMQTTGateway mac: %s" CR), WiFi.macAddress().c_str());
  Logger.debug(0, F("OpenMQTTGateway ip: %s" CR), WiFi.localIP().toString().c_str());
  Logger.debug(0, F("OpenMQTTGateway index %d" CR), cnt_index);
  Logger.debug(0, F("OpenMQTTGateway mqtt topic: %s" CR), mqtt_topic);
#ifdef ZmqttDiscovery
  Logger.debug(0, F("OpenMQTTGateway mqtt discovery prefix: %s" CR), discovery_prefix);
#endif
  Logger.debug(0, F("OpenMQTTGateway gateway name: %s" CR), gateway_name);
#if !MQTT_BROKER_MODE
  Logger.debug(0, F("OpenMQTTGateway mqtt server: %s" CR), cnt_parameters_array[cnt_index].mqtt_server);
  Logger.debug(0, F("OpenMQTTGateway mqtt port: %s" CR), cnt_parameters_array[cnt_index].mqtt_port);
  Logger.debug(0, F("OpenMQTTGateway mqtt user: %s" CR), cnt_parameters_array[cnt_index].mqtt_user);
  Logger.debug(0, F("OpenMQTTGateway secure connection: %s" CR), cnt_parameters_array[cnt_index].isConnectionSecure ? "true" : "false");
  Logger.debug(0, F("OpenMQTTGateway validate cert: %s" CR), cnt_parameters_array[cnt_index].isCertValidate ? "true" : "false");
#endif
...
#endif
  Logger.debug(0, F("mqtt_max_payload_size: %d" CR), mqtt_max_payload_size);
  SYSConfig.offline ? Logger.notice(0, F("Offline enabled" CR)) : Logger.notice(0, F("Offline disabled" CR));
  char jsonChar[100];
  serializeJson(modules, jsonChar, measureJson(modules) + 1);
  Logger.notice(0, F("OpenMQTTGateway modules: %s" CR), jsonChar);
  Logger.notice(0, F("************** Setup OpenMQTTGateway end **************" CR));
}
...
bool wifi_reconnect_bypass() {
  ...
  if (omg_blufi_ble_connected) {
    Logger.notice(0, F("BLUFI is connected, bypassing wifi reconnect" CR));
...
#endif
    Logger.notice(0, F("Attempting Wifi connection with saved AP: %d" CR), wifi_autoreconnect_cnt);
...
void setOTA() {
  ...
  ArduinoOTA.onStart([]() {
    Logger.debug(0, F("Start OTA, lock other functions" CR));
...
#  endif
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    Logger.debug(0, F("Progress: %u%%\r" CR), (progress / (total / 100)));
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    ...
    if (error == OTA_AUTH_ERROR)
      Logger.error(0, F("Auth Failed" CR));
    else if (error == OTA_BEGIN_ERROR)
      Logger.error(0, F("Begin Failed" CR));
    else if (error == OTA_CONNECT_ERROR)
      Logger.error(0, F("Connect Failed" CR));
    else if (error == OTA_RECEIVE_ERROR)
      Logger.error(0, F("Receive Failed" CR));
    else if (error == OTA_END_ERROR)
      Logger.error(0, F("End Failed" CR));
    ESPRestart(6);
  });
  ArduinoOTA.begin();
}
...
void setupTLS(int index) {
  ...
  Logger.notice(0, F("cnt index used: %d" CR), index);
  if (!cnt_parameters_array[index].isCertValidate) {
    Logger.notice(0, F("Disabling cert validation" CR));
    sClient->setInsecure();
  } else {
    Logger.notice(0, F("Enabling cert validation" CR));
#  if defined(ESP32)
    if (cnt_parameters_array[index].server_cert.length() > MIN_CERT_LENGTH) {
      ...
      Logger.notice(0, F("Server cert found from cert array" CR));
    } else if (strlen(ss_server_cert) > MIN_CERT_LENGTH) {
      sClient->setCACert(ss_server_cert);
      Logger.notice(0, F("Server cert found from ss_server_cert" CR));
    } else {
      Logger.error(0, F("No server cert found" CR));
      gatewayState = GatewayState::ERROR;
    }

#    if AWS_IOT
    if (strcmp(cnt_parameters_array[index].mqtt_port, "443") == 0) {
      Logger.notice(0, F("Using ALPN" CR));
...
#    if MQTT_SECURE_SIGNED_CLIENT
    if (cnt_parameters_array[index].client_cert.length() > MIN_CERT_LENGTH) {
      ...
      Logger.notice(0, F("Client cert found from cert array" CR));
    } else if (strlen(ss_client_cert) > MIN_CERT_LENGTH) {
      sClient->setCertificate(ss_client_cert);
      Logger.notice(0, F("Client cert found from ss_client_cert" CR));
    } else {
      Logger.error(0, F("No client cert found" CR));
      gatewayState = GatewayState::ERROR;
    }
    if (cnt_parameters_array[index].client_key.length() > MIN_CERT_LENGTH) {
      ...
      Logger.notice(0, F("Client key found from cert array" CR));
    } else if (strlen(ss_client_key) > MIN_CERT_LENGTH) {
      sClient->setPrivateKey(ss_client_key);
      Logger.notice(0, F("Client key found from ss_client_key" CR));
    } else {
      Logger.error(0, F("No client key found" CR));
      gatewayState = GatewayState::ERROR;
    }
#    endif
#  elif defined(ESP8266)
...
void ESPRestart(byte reason) {
  ...
  Logger.notice(0, F("Restarting secondary module : %s" CR), restartCmdStr.c_str());
  ...
  Logger.warning(0, F("Rebooting for reason code %d" CR), reason);
#if defined(ESP32)
  ESP.restart();
#elif defined(ESP8266)
  ESP.reset();
...
void setupWiFiFromBuild() {
  ...
  Logger.debug(0, F("Connecting to %s" CR), wifi_ssid);
#  ifdef wifi_ssid1
  wifiMulti.addAP(wifi_ssid1, wifi_password1);
  Logger.debug(0, F("Connecting to %s" CR), wifi_ssid1);
#  endif
  delay(10);
...
#  ifdef NetworkAdvancedSetup
  ...
  if (!WiFi.config(ip_adress, gateway_adress, subnet_adress, dns_adress)) {
    Logger.error(0, F("Wifi STA Failed to configure" CR));
    gatewayState = GatewayState::ERROR;
  }
...
#  endif
  ...
  Logger.notice(0, F("WiFi ok with manual config credentials" CR));
...
void saveConfigCallback() {
  Logger.debug(0, F("Should save config" CR));
...
void blockingWaitForReset() {
  if (digitalRead(TRIGGER_GPIO) == LOW) {
    delay(50);
    if (digitalRead(TRIGGER_GPIO) == LOW) {
      Logger.debug(0, F("Trigger button Pressed" CR));
      delay(3000); // reset delay hold
      if (digitalRead(TRIGGER_GPIO) == LOW) {
        Logger.notice(0, F("Button Held" CR));
...
#    endif
        ...
        if (SPIFFS.begin()) {
          Logger.debug(0, F("mounted file system" CR));
          if (SPIFFS.exists("/config.json")) {
            Logger.notice(0, F("Erasing ESP Config, restarting" CR));
            erase(true);
          }
        }
        delay(30000);
        if (digitalRead(TRIGGER_GPIO) == LOW) {
          Logger.notice(0, F("Going into failsafe mode without peripherals" CR));
...
void saveConfig() {
  Logger.debug(0, F("saving configs" CR));
...
#  endif
  ...
  Logger.notice(0, F("Total size: %d" CR), totalSize);
...
#  endif
  ...
  if (!configFile) {
    Logger.error(0, F("failed to open config file for writing" CR));
    gatewayState = GatewayState::ERROR;
  }
...
bool loadConfigFromFlash() {
  Logger.debug(0, F("mounting FS..." CR));
  bool result = false;

  if (SPIFFS.begin()) {
    Logger.debug(0, F("mounted file system" CR));
  } else {
    Logger.warning(0, F("failed to mount FS -> formating" CR));
    SPIFFS.format();
    if (SPIFFS.begin())
      Logger.debug(0, F("mounted file system after formating" CR));
  }
  if (SPIFFS.exists("/config.json")) {
    //file exists, reading and loading
    Logger.debug(0, F("reading config file" CR));
    File configFile = SPIFFS.open("/config.json", "r");
    if (configFile) {
      Logger.debug(0, F("opened config file" CR));
      DynamicJsonDocument json(configFile.size() * 2);
      auto error = deserializeJson(json, configFile);
      if (error) {
        Logger.error(0, F("deserialize config failed: %s, buffer capacity: %u" CR), error.c_str(), json.capacity());
        gatewayState = GatewayState::ERROR;
      }
      if (!json.isNull()) {
...
#    ifdef ESP32
            ...
            if (hash == GITHUB_OTA_SERVER_CERT_HASH) {
              // Do nothing
              Logger.warning(0, F("Old Github OTA server detected, skipping" CR));
            } else {
              Logger.notice(0, F("OTA server cert hash: %s" CR), hash.c_str());
...
#    endif
          ...
          if (json.containsKey(key)) {
            ...
            Logger.warning(0, F("valid_cnt not found, assuming connection is valid" CR));
...
#  endif
        ...
        Logger.warning(0, F("failed to load json config" CR));
      }
      configFile.close();
    }
  } else {
    Logger.notice(0, F("No config file found defining default values" CR));
#  ifdef USE_MAC_AS_GATEWAY_NAME
    String s = WiFi.macAddress();
...
//set static IP
#  ifdef NetworkAdvancedSetup
  Logger.debug(0, F("Adv wifi cfg" CR));
  IPAddress ip_adress;
  IPAddress gateway_adress;
  IPAddress subnet_adress;
  IPAddress dns_adress;
...
#  endif
  ...
  {
    Logger.notice(0, F("Connect your phone to WIFI AP: %s with PWD: %s" CR), WifiManager_ssid, ota_pass);
    gatewayState = GatewayState::ONBOARDING;
    //fetches ssid and pass and tries to connect
    //if it does not connect it starts an access point with the specified name
    //and goes into a blocking loop awaiting configuration
    if (!wifiManager.autoConnect(WifiManager_ssid, ota_pass)) {
      Logger.warning(0, F("failed to connect and hit timeout" CR));
      delay(3000);
...
#    ifdef NetworkAdvancedSetup
  ...
  Logger.debug(0, F("Adv eth cfg" CR));
  ETH.config(ip, gateway, subnet, Dns);
  ethBeginSuccess = ETH.begin();
#    else
  Logger.notice(0, F("Spl eth cfg" CR));
  ethBeginSuccess = ETH.begin();
#    endif
  if (ethBeginSuccess) {
    Logger.notice(0, F("Ethernet started" CR));
    while (!ethConnected && failure_number_ntwk <= maxConnectionRetryNetwork) {
      delay(500);
      Logger.notice(0, F("." CR));
      failure_number_ntwk++;
    }
  } else {
    Logger.error(0, F("Ethernet not started" CR));
    gatewayState = GatewayState::ERROR;
  }
}
...
void WiFiEvent(WiFiEvent_t event) {
  switch (event) {
    case ARDUINO_EVENT_ETH_START:
      Logger.debug(0, F("Ethernet Started" CR));
      ETH.setHostname(gateway_name);
      break;
    case ARDUINO_EVENT_ETH_CONNECTED:
      Logger.notice(0, F("Ethernet Connected" CR));
      break;
    case ARDUINO_EVENT_ETH_GOT_IP:
      Logger.notice(0, F("OpenMQTTGateway Ethernet MAC: %s" CR), ETH.macAddress().c_str());
      Logger.notice(0, F("OpenMQTTGateway Ethernet IP: %s" CR), ETH.localIP().toString().c_str());
      Logger.notice(0, F("OpenMQTTGateway Ethernet link speed: %d Mbps" CR), ETH.linkSpeed());
      gatewayState = GatewayState::NTWK_CONNECTED;
      ethConnected = true;
      break;
    case ARDUINO_EVENT_ETH_DISCONNECTED:
      Logger.warning(0, F("Ethernet Disconnected" CR));
      ethConnected = false;
      break;
    case ARDUINO_EVENT_ETH_STOP:
      Logger.warning(0, F("Ethernet Stopped" CR));
      ethConnected = false;
      break;
    default:
      break;
...
void sleep() {
  if (SYSConfig.powerMode < PowerMode::INTERVAL)
    return;
  Logger.notice(0, F("Entering deep sleep" CR));
...
#  endif
  Logger.debug(0, F("Deactivating ESP32 components" CR));
#  ifdef ZgatewayBT
  stopProcessing();
  ProcessLock = true;
#  endif
...
#  pragma GCC diagnostic pop
  esp_wifi_stop();
#  ifdef ESP32_EXT0_WAKE_PIN
  Logger.notice(0, F("Entering deep sleep, EXT0 Wakeup by pin : %l." CR), ESP32_EXT0_WAKE_PIN);
#  endif
#  ifdef ESP32_EXT1_WAKE_PIN
  Logger.notice(0, F("Entering deep sleep, EXT1 Wakeup by pin : %l." CR), ESP32_EXT1_WAKE_PIN);
#  endif
  if (SYSConfig.powerMode == PowerMode::ACTION) {
    esp_deep_sleep_start();
  } else if (SYSConfig.powerMode == PowerMode::INTERVAL) {
    Logger.notice(0, F("Entering deep sleep for %l us." CR), DEEP_SLEEP_IN_US);
...
#  endif
  ...
  if (ethConnected || WiFi.status() == WL_CONNECTED) {
    ...
    Logger.warning(0, F("Network disconnected" CR));
...
#ifdef Zgateway2G
    if (_2GtoX())
      Logger.debug(0, F("2GtoMQTT OK" CR));
#endif
#ifdef ZgatewayRFM69
    if (RFM69toX())
      Logger.debug(0, F("RFM69toMQTT OK" CR));
#endif
#ifdef ZactuatorFASTLED
    FASTLEDLoop();
#endif
...
void erase(bool restart) {
  ...
  Logger.notice(0, F("Erasing secondary module config: %s" CR), eraseCmdStr.c_str());
  receivingDATA(subjectMQTTtoSYSsetSecondaryModule, eraseCmdStr.c_str());
  delay(2000);
#endif
  Logger.debug(0, F("Formatting requested, result: %d" CR), SPIFFS.format());
...
String stateMeasures() {
  ...
  if (freeMem < MinimumMemory) {
    Logger.error(0, F("Not enough memory %d, restarting" CR), freeMem);
    gatewayState = GatewayState::ERROR;
    ESPRestart(8);
  }
#endif
...
#  endif
  ...
  Logger.notice(0, F("SYS json: %s" CR), output.c_str());
  return output;
}
...
void storeSignalValue(uint64_t MQTTvalue) {
  ...
  int o = getMin();
  Logger.debug(0, F("Min ind: %d" CR), o);
  ...
  Logger.debug(0, F("store code : %u / %u" CR), (unsigned long)receivedSignal[o].value, receivedSignal[o].time);
  Logger.debug(0, F("Col: val/timestamp" CR));
  for (int i = 0; i < struct_size; i++) {
    Logger.debug(0, F("mem code : %u / %u" CR), (unsigned long)receivedSignal[i].value, receivedSignal[i].time);
  }
}
...
void receivingDATA(const char* topicOri, const char* datacallback) {
  ...
  if (error || jsondata.isNull()) {
    Logger.error(0, F("deserialize MQTT data failed: %s" CR), error.c_str());
    gatewayState = GatewayState::ERROR;
    return;
  }
  if (topicOri == nullptr || strcmp(topicOri, "") == 0) {
    if (jsondata.containsKey("target") && jsondata["target"].is<const char*>()) {
      ...
      Logger.debug(0, F("BUS Msg target: %s" CR), strTopicOri.c_str());
    } else if (jsondata.containsKey("origin") && jsondata["origin"].is<const char*>()) {
      strTopicOri = jsondata["origin"].as<const char*>();
      Logger.debug(0, F("BUS Msg origin: %s" CR), strTopicOri.c_str());
    }
  } else {
...
#endif
    Logger.debug(0, F("MQTT Msg topic: %s" CR), strTopicOri.c_str());
  }
  ...
  if (!jsondata.isNull()) { // json object ok -> json decoding
    ...
    //Logger.notice(0, F("[ MQTT->OMG ]: %s" CR), buffer.c_str());
...
bool checkForUpdates() {
  Logger.notice(0, F("Update check, free heap: %d"), ESP.getFreeHeap());
  HTTPClient http;
...
#      if !MQTT_BROKER_MODE
  if (cnt_parameters_array[cnt_index].ota_server_cert.length() > MIN_CERT_LENGTH) {
    Logger.notice(0, F("Using memory cert" CR));
    ota_cert = cnt_parameters_array[cnt_index].ota_server_cert;
  } else
#      endif
  {
    Logger.notice(0, F("Using config cert" CR));
  ...
  if (httpCode > 0) { //Check for the returning code
    ...
    Logger.debug(0, F("HttpCode %d" CR), httpCode);
    Logger.debug(0, F("Payload %s" CR), payload.c_str());
  } else {
    Logger.error(0, F("Error on HTTP request"));
    gatewayState = GatewayState::ERROR;
  }
  http.end(); //Free the resources
  Logger.notice(0, F("Update check done, free heap: %d"), ESP.getFreeHeap());
  if (jsondata.containsKey("latest_version")) {
    ...
    Logger.debug(0, F("Update file found on server" CR));
    return true;
  } else {
    Logger.debug(0, F("No update file found on server" CR));
    return false;
  }
}
    ...
    if (version && ((strlen(version) != strlen(OMG_VERSION)) || strcmp(version, OMG_VERSION) != 0)) {
      ...
      if (url) {
        if (!strstr((url + (strlen(url) - 5)), ".bin")) {
          Logger.error(0, F("Invalid firmware extension" CR));
          gatewayState = GatewayState::ERROR;
          return;
        }
#  if MQTT_HTTPS_FW_UPDATE_USE_PASSWORD > 0
        ...
        if (pwd) {
          if (strcmp(pwd, ota_pass) != 0) {
            Logger.error(0, F("Invalid OTA password" CR));
            gatewayState = GatewayState::ERROR;
            return;
          }
        } else {
          Logger.error(0, F("No password sent" CR));
          gatewayState = GatewayState::ERROR;
          return;
        }
#  endif
#  ifdef ESP32
        ...
        url = systemUrl.c_str();
        Logger.notice(0, F("Using system OTA url with latest version %s" CR), url);
      } else if (strcmp(version, "dev") == 0) {
        systemUrl = String(RELEASE_LINK_DEV) + ENV_NAME + "-firmware.bin";
        url = systemUrl.c_str();
        Logger.notice(0, F("Using system OTA url with dev version %s" CR), url);
      } else if (version[0] == 'v') {
        systemUrl = String(RELEASE_LINK) + version + "/" + ENV_NAME + "-firmware.bin";
        url = systemUrl.c_str();
        Logger.notice(0, F("Using system OTA url with defined version %s" CR), url);
#  endif
      } else {
        Logger.error(0, F("Invalid URL" CR));
        gatewayState = GatewayState::ERROR;
        return;
      }
#  ifdef ESP32
...
#    endif
#  endif
      Logger.warning(0, F("Starting firmware update" CR));
      ...
      Logger.notice(0, F("OTA cert: %s" CR), ota_cert.c_str());
...
#  endif
      ...
      if (strstr(url, "http:")) {
        Logger.notice(0, F("Http update" CR));
...
#  endif
        ...
        case HTTP_UPDATE_NO_UPDATES:
          Logger.notice(0, F("HTTP_UPDATE_NO_UPDATES" CR));
          break;

        case HTTP_UPDATE_OK:
          Logger.notice(0, F("HTTP_UPDATE_OK" CR));
...
void readCntParameters(int index) {
  if (index < 0 || index > 2) {
    Logger.warning(0, F("Invalid cnt index" CR));
    return;
  }
  StaticJsonDocument<JSON_MSG_BUFFER> jsonBuffer;
  JsonObject jsondata = jsonBuffer.to<JsonObject>();
...
void XtoSYS(const char* topicOri, JsonObject& SYSdata) { // json object decoding
  if (cmpToMainTopic(topicOri, subjectMQTTtoSYSset)) {
    ...
    Logger.debug(0, F("MQTTtoSYS json" CR));
    if (SYSdata.containsKey("cmd")) {
      const char* cmd = SYSdata["cmd"];
      Logger.notice(0, F("Command: %s" CR), cmd);
...
#  endif
        Logger.notice(0, F("RGB brightness: %d" CR), SYSConfig.rgbbrightness);
        publishState = true;
      } else {
        Logger.warning(0, F("RGB brightness value invalid - ignoring command" CR));
      }
    }
#endif
#ifdef ZmqttDiscovery
    if (SYSdata.containsKey("ohdisc") && SYSdata["ohdisc"].is<bool>()) {
      ...
      Logger.notice(0, F("OpenHAB discovery: %T" CR), SYSConfig.ohdiscovery);
      publishState = true;
    }
#endif
...
#  endif
      ...
      Logger.warning(0, F("Attempting connection to new AP %s" CR), (const char*)SYSdata["wifi_ssid"]);
      ...
      if (WiFi.status() != WL_CONNECTED) {
        Logger.warning(0, F("Failed to connect to new AP; falling back" CR));
        WiFi.disconnect(true);
...
#  ifdef MQTTsetMQTT
    ...
    if (SYSdata.containsKey("cnt_index") && SYSdata["cnt_index"].is<int>()) {
      if (SYSdata["cnt_index"].as<int>() < 0 || SYSdata["cnt_index"].as<int>() > 2) {
        Logger.warning(0, F("Invalid cnt index provided - ignoring command" CR));
        return;
      }
      ...
      Logger.notice(0, F("MQTT cnt index %d" CR), cnt_index);
      ...
      if (SYSdata.containsKey("mqtt_server_cert") && SYSdata["mqtt_server_cert"].is<const char*>()) {
        ...
        Logger.debug(0, F("Assigning server cert %s" CR), generateHash(cnt_parameters_array[cnt_index].server_cert).c_str());
        cnt_parameters_array[cnt_index].validConnection = false;
      }
      if (SYSdata.containsKey("mqtt_client_cert") && SYSdata["mqtt_client_cert"].is<const char*>()) {
        ...
        Logger.debug(0, F("Assigning client cert %s" CR), generateHash(cnt_parameters_array[cnt_index].client_cert).c_str());
        cnt_parameters_array[cnt_index].validConnection = false;
      }
      if (SYSdata.containsKey("mqtt_client_key") && SYSdata["mqtt_client_key"].is<const char*>()) {
        ...
        Logger.debug(0, F("Assigning client key %s" CR), generateHash(cnt_parameters_array[cnt_index].client_key).c_str());
        cnt_parameters_array[cnt_index].validConnection = false;
      }
      if (SYSdata.containsKey("ota_server_cert") && SYSdata["ota_server_cert"].is<const char*>()) {
        ...
        Logger.debug(0, F("Assigning OTA server cert %s" CR), generateHash(cnt_parameters_array[cnt_index].ota_server_cert).c_str());
      }
...
// Disconnect MQTT
    ...
    if (SYSdata.containsKey("powermode") && SYSdata["powermode"].is<int>()) {
      ...
      Logger.notice(0, F("Power mode: %d" CR), SYSConfig.powerMode);
    }
#if USE_BLUFI
    if (SYSdata.containsKey("blufi") && SYSdata["blufi"].is<bool>()) {
      ...
      publishState = true;
      Logger.notice(0, F("Blufi: %T" CR), SYSConfig.blufi);
    }
#endif
    if (SYSdata.containsKey("disc")) {
      if (SYSdata["disc"].is<bool>()) {
        ...
        Logger.warning(0, F("Discovery command not a boolean" CR));
      }
      Logger.notice(0, F("Discovery state: %T" CR), SYSConfig.discovery);
    }
