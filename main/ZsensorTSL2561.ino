...
void displaySensorDetails(void) {
  ...
  Logger.debug(0, F("------------------------------------" CR));
  Logger.debug(0, ("Sensor: %s" CR), sensor.name);
  Logger.debug(0, ("Driver Ver: %s" CR), sensor.version);
  Logger.debug(0, ("Unique ID: %s" CR), sensor.sensor_id);
  Logger.debug(0, ("Max Value: %s lux" CR), sensor.max_value);
  Logger.debug(0, ("Min Value: %s lux" CR), sensor.min_value);
  Logger.debug(0, ("Resolution: %s lux" CR), sensor.resolution);
  Logger.debug(0, F("------------------------------------" CR));
  delay(500);
}

void setupZsensorTSL2561() {
  Logger.notice(0, F("Setup TSL2561 on adress: %H" CR), TSL2561_ADDR_FLOAT);
  Wire.begin();
  Wire.beginTransmission(TSL2561_ADDR_FLOAT);

  if (!tsl.begin()) {
    Logger.error(0, F("No TSL2561 detected" CR));
  }
  ...
  Logger.debug(0, F("TSL2561 Initialized. Printing detials now." CR));
...
void MeasureLightIntensityTSL2561() {
  if (millis() > (timetsl2561 + TimeBetweenReadingtsl2561)) {
    ...
    Logger.debug(0, F("Creating TSL2561 buffer" CR));
    ...
    // if event.light == 0 the sensor is clipping, do not send
    {
      if (persisted_lux != event.light || tsl2561_always) {
        ...
        Logger.debug(0, F("Same lux value, do not send" CR));
      }
    } else {
      Logger.error(0, F("Failed to read from TSL2561" CR));
    }
  }
}
#endif
