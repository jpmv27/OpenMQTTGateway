...
void ONOFFConfig_fromJson(JsonObject& ONOFFdata) {
  ...
  if (ONOFFdata.containsKey("erase") && ONOFFdata["erase"].as<bool>()) {
    ...
    if (preferences.isKey("ONOFFConfig")) {
      ...
      Logger.notice(OMG_LOGID, F("ONOFF config erase result: %d" CR), result);
      preferences.end();
      return; // Erase prevails on save, so skipping save
    } else {
      Logger.notice(OMG_LOGID, F("ONOFF config not found" CR));
      preferences.end();
    }
  }
  if (ONOFFdata.containsKey("save") && ONOFFdata["save"].as<bool>()) {
    ...
    preferences.end();
    Logger.notice(OMG_LOGID, F("ONOFF Config_save: %s, result: %d" CR), conf.c_str(), result);
  }
}
...
void ONOFFConfig_load() {
  ...
  if (preferences.isKey("ONOFFConfig")) {
    ...
    preferences.end();
    if (error) {
      Logger.error(OMG_LOGID, F("ONOFF config deserialization failed: %s, buffer capacity: %u" CR), error.c_str(), jsonBuffer.capacity());
      return;
    }
    if (jsonBuffer.isNull()) {
      Logger.warning(OMG_LOGID, F("ONOFF config is null" CR));
      return;
    }
    JsonObject jo = jsonBuffer.as<JsonObject>();
    ONOFFConfig_fromJson(jo);
    Logger.notice(OMG_LOGID, F("ONOFF config loaded" CR));
  } else {
    preferences.end();
    Logger.notice(OMG_LOGID, F("ONOFF config not found" CR));
  }
...
#  ifdef ESP32
  ONOFFConfig_init();
  ONOFFConfig_load();
  Logger.notice(OMG_LOGID, F("Target state on restart: %T" CR), ONOFFConfig.ONOFFState);
  Logger.notice(OMG_LOGID, F("Use last state on restart: %T" CR), ONOFFConfig.useLastStateOnStart);
#  endif
...
#  endif
  ...
  Logger.debug(OMG_LOGID, F("ZactuatorONOFF setup done" CR));
}

#  if jsonReceiving
void XtoONOFF(const char* topicOri, JsonObject& ONOFFdata) {
  if (cmpToMainTopic(topicOri, subjectMQTTtoONOFF)) {
    Logger.debug(OMG_LOGID, F("MQTTtoONOFF json data analysis" CR));
    int boolSWITCHTYPE = ONOFFdata["cmd"] | 99;
    int gpio = ONOFFdata["gpio"] | ACTUATOR_ONOFF_GPIO;
    if (boolSWITCHTYPE != 99) {
      Logger.notice(OMG_LOGID, F("MQTTtoONOFF boolSWITCHTYPE ok: %d" CR), boolSWITCHTYPE);
      Logger.notice(OMG_LOGID, F("GPIO number: %d" CR), gpio);
      pinMode(gpio, OUTPUT);
...
#    endif
      ...
      if (ONOFFdata["cmd"] == "high_pulse") {
        Logger.notice(OMG_LOGID, F("MQTTtoONOFF high_pulse ok" CR));
        Logger.notice(OMG_LOGID, F("GPIO number: %d" CR), gpio);
        int pulselength = ONOFFdata["pulse_length"] | 500;
        Logger.notice(OMG_LOGID, F("Pulse length: %d ms" CR), pulselength);
        pinMode(gpio, OUTPUT);
        digitalWrite(gpio, HIGH);
        delay(pulselength);
        digitalWrite(gpio, LOW);
      } else if (ONOFFdata["cmd"] == "low_pulse") {
        Logger.notice(OMG_LOGID, F("MQTTtoONOFF low_pulse ok" CR));
        Logger.notice(OMG_LOGID, F("GPIO number: %d" CR), gpio);
        int pulselength = ONOFFdata["pulse_length"] | 500;
        Logger.notice(OMG_LOGID, F("Pulse length: %d ms" CR), pulselength);
        pinMode(gpio, OUTPUT);
        ...
        Logger.error(OMG_LOGID, F("MQTTtoONOFF failed json read" CR));
      }
    }
  }
  if (cmpToMainTopic(topicOri, subjectMQTTtoONOFFset)) {
    Logger.debug(OMG_LOGID, F("MQTTtoONOFF json set" CR));
    /*
...
#    if defined(ESP32) && !defined(NO_INT_TEMP_READING)
  for (;;) {
    ...
    Logger.debug(OMG_LOGID, F("Internal temperature of the ESP32 %F" CR), internalTempc);
    ...
    if (internalTempc > MAX_TEMP_ACTUATOR && previousInternalTempc > MAX_TEMP_ACTUATOR) {
      if (digitalRead(ACTUATOR_ONOFF_GPIO) == ACTUATOR_ON) { // This could be with the previous condition, but it is better to trigger the digitalRead only if the previous condition is met to avoid the digitalRead
        Logger.error(OMG_LOGID, F("[ActuatorONOFF] OverTemperature detected ( %F > %F ) switching OFF Actuator" CR), internalTempc, MAX_TEMP_ACTUATOR);
        ActuatorTrigger();
#      ifdef LED_ACTUATOR_ONOFF
...
void overLimitCurrent(float RN8209current) {
  ...
  Logger.debug(OMG_LOGID, F("RN8209 Current %F" CR), RN8209current);
  ...
  if (RN8209current > MAX_CURRENT_ACTUATOR && RN8209previousCurrent > MAX_CURRENT_ACTUATOR) {
    if (digitalRead(ACTUATOR_ONOFF_GPIO) == ACTUATOR_ON) { // This could be with the previous condition, but it is better to trigger the digitalRead only if the previous condition is met to avoid the digitalRead
      Logger.error(OMG_LOGID, F("[ActuatorONOFF] OverCurrent detected ( %F > %F ) switching OFF Actuator" CR), RN8209current, MAX_CURRENT_ACTUATOR);
      ActuatorTrigger();
#    ifdef LED_ACTUATOR_ONOFF
      ledManager.setMode(LED_ACTUATOR_ONOFF, 0, LEDManager::Mode::STATIC, LED_ERROR_COLOR, -1);
#    endif
...
void ActuatorTrigger() {
  ...
  Logger.debug(OMG_LOGID, F("Actuator triggered %d" CR), level);
