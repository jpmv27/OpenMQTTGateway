...
void setupZsensorHTU21() {
  ...
  Logger.notice(0, F("HTU21 Initialized - begin()" CR));
...
void MeasureTempHum() {
  if (millis() > (timehtu21 + TimeBetweenReadinghtu21)) {
    Logger.debug(0, F("Read HTU21 Sensor" CR));
    ...
    if (HtuTempC >= 998 || HtuHum >= 998) {
      Logger.error(0, F("Failed to read from sensor HTU21!" CR));
      return;
    }
    ...
    if (isnan(HtuTempC) || isnan(HtuHum)) {
      ...
      Logger.notice(0, F("Creating HTU21 buffer" CR));
