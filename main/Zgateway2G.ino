...
void setup2G() {
  Logger.notice(0, F("_2G_TX_GPIO: %d " CR), _2G_TX_GPIO);
  Logger.notice(0, F("_2G_RX_GPIO: %d " CR), _2G_RX_GPIO);
  setupGSM(false);
  Logger.debug(0, F("Zgateway2G setup done " CR));
}

void setupGSM(bool deleteSMS) {
  Logger.debug(0, F("Init 2G module: %d" CR), _2G_PWR_GPIO);
  delay(1000);
  // Power-cycle the module to reset it.
  A6l.powerCycle(_2G_PWR_GPIO);
  Logger.notice(0, F("waiting for network connection at bd: %d" CR), _2G_MODULE_BAUDRATE);
  A6l.blockUntilReady(_2G_MODULE_BAUDRATE);
  Logger.notice(0, F("A6/A7 gsm ready" CR));
  signalStrengthAnalysis();
  delay(1000);
  // deleting all sms
  if (deleteSMS) {
    if (A6l.deleteSMS(1, 4) == A6_OK) {
      Logger.notice(0, F("delete SMS OK" CR));
    } else {
      Logger.error(0, F("delete SMS KO" CR));
    }
  }
}
...
void signalStrengthAnalysis() {
  ...
  Logger.debug(0, F("Signal strength: %d" CR), signalStrength);
  if (signalStrength < _2G_MIN_SIGNAL || signalStrength > _2G_MAX_SIGNAL) {
    Logger.debug(0, F("Signal too low restart the module" CR));
...
bool _2GtoX() {
  ...
  Logger.debug(0, F("Creating SMS  buffer" CR));
  ...
  for (int i = 0; i < unreadSMSNum; i++) {
    Logger.notice(0, F("New  message at index: %d" CR), unreadSMSNum);
    ...
    Logger.debug(0, F("Adv data 2GtoMQTT" CR));
...
void Xto2G(const char* topicOri, const char* datacallback) {
  ...
  if (cmpToMainTopic(topicOri, subjectMQTTto2G)) {
    Logger.debug(0, F("MQTTto2G data analysis" CR));
    // 2G DATA ANALYSIS
    String phone_number = "";
    ...
    if (pos0 != -1) {
      ...
      Logger.notice(0, F("MQTTto2G phone: %s" CR), (char*)phone_number.c_str());
      Logger.notice(0, F("MQTTto2G sms: %s" CR), (char*)data.c_str());
      if (A6l.sendSMS(phone_number, data) == A6_OK) {
        Logger.notice(0, F("SMS OK" CR));
      ...
      Logger.error(0, F("MQTTto2G Fail reading phone number" CR));
    }
  }
}
#  endif
...
void Xto2G(const char* topicOri, JsonObject& SMSdata) {
  if (cmpToMainTopic(topicOri, subjectMQTTto2G)) {
    ...
    Logger.debug(0, F("MQTTto2G json data analysis" CR));
    if (sms && phone) {
      Logger.notice(0, F("MQTTto2G phone: %s" CR), phone);
      Logger.notice(0, F("MQTTto2G sms: %s" CR), sms);
      ...
      Logger.error(0, F("MQTTto2G failed json read" CR));
    }
  }
}
#  endif
