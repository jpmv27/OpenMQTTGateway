...
void setupZsensorMQ2() {
  Logger.notice(OMG_LOGID, F("Setup MQ2 detection on pin: %d"), MQ2SENSORDETECTPIN);
  ...
  Logger.notice(OMG_LOGID, F("Starting MQ2 calibration on pin: %d"), MQ2SENSORADCPIN);
  ...
  Logger.debug(OMG_LOGID, F("MQ2 Initialized."));
}
...
void MeasureGasMQ2() {
  if (millis() > (timemq2 + TimeBetweenReadingmq2)) {
    ...
    Logger.debug(OMG_LOGID, F("Creating MQ2 buffer"));
