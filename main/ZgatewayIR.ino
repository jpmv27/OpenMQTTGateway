...
void setupIR() {
  ...
  Logger.notice(0, F("IR_EMITTER_GPIO: %d " CR), IR_EMITTER_GPIO);
  Logger.notice(0, F("IR_RECEIVER_GPIO: %d " CR), IR_RECEIVER_GPIO);
  Logger.debug(0, F("ZgatewayIR setup done " CR));
}
...
void IRtoX() {
  ...
  if (irrecv.decode(&results)) {
    Logger.debug(0, F("Creating IR buffer" CR));
    StaticJsonDocument<JSON_MSG_BUFFER> IRdataBuffer;
    JsonObject IRdata = IRdataBuffer.to<JsonObject>();

    Logger.debug(0, F("Rcv. IR" CR));
#  ifdef ESP32
    Logger.debug(0, F("IR Task running on core :%d" CR), xPortGetCoreID());
#  endif
...
#  ifdef RawDirectForward
    ...
    Logger.debug(0, F("raw redirected" CR));
#  endif
    ...
    if ((pubIRunknownPrtcl == false && IRdata["protocol"].as<int>() == -1)) { // don't publish unknown IR protocol
      Logger.notice(0, F("--no pub unknwn prt--" CR));
    } else if (!isAduplicateSignal(MQTTvalue) && MQTTvalue != 0) { // conditions to avoid duplications of IR -->MQTT
      Logger.debug(0, F("Adv data IRtoMQTT" CR));
      IRdata["origin"] = subjectIRtoMQTT;
      enqueueJsonObject(IRdata);
      Logger.debug(0, F("Store val: %D" CR), MQTTvalue);
      storeSignalValue(MQTTvalue);
      if (repeatIRwMQTT) {
        Logger.debug(0, F("Pub. IR for rpt" CR));
...
void XtoIR(const char* topicOri, JsonObject& IRdata) {
  if (cmpToMainTopic(topicOri, subjectMQTTtoIR)) {
    Logger.debug(0, F("MQTTtoIR json" CR));
    ...
    if (hex) { // we privilegiate the hex usage over the value one (less risk of error)
      Logger.debug(0, F("hex: %s" CR), hex);
      data = getUInt64fromHex(hex);
    }
    if (data != 0 || raw) {
      Logger.debug(0, F("MQTTtoIR value || raw  detected" CR));
      irrecv.disableIRIn();
...
#    endif
        ...
        Logger.debug(0, F("Using Identified Protocol: %s  bits: %d repeat: %d" CR), protocol_name, valueBITS, valueRPT);
        signalSent = sendIdentifiedProtocol(protocol_name, data, hex, valueBITS, valueRPT);
      } else {
        Logger.debug(0, F("Using NEC protocol" CR));
        Logger.notice(0, F("Sending IR signal with %s" CR), protocol_name);
        if (valueBITS == 0)
          valueBITS = NEC_BITS;
      ...
      if (signalSent) { // we acknowledge the sending by publishing the value to an acknowledgement topic, for the moment even if it is a signal repetition we acknowledge also
        Logger.notice(0, F("MQTTtoIR OK" CR));
      ...
      Logger.error(0, F("MQTTtoIR failed json read" CR));
    }
  }
}
#  endif
...
bool sendIdentifiedProtocol(const char* protocol_name, uint64_t data, const char* hex, unsigned int valueBITS, uint16_t valueRPT) {
  ...
  if (hex) {
    ...
    for (int i = 0; i < valueBITS; i++) {
      Logger.debug(0, F("%x"), dataarray[i]);
    }
  }
#  ifdef IR_WHYNTER
...
#  ifdef IR_COOLIX
  if (strcmp(protocol_name, "COOLIX") == 0) {
    Logger.debug(0, F("Sending %s:" CR), protocol_name);
    Logger.notice(0, F("Sending IR signal with %s" CR), protocol_name);
    if (valueBITS == 0)
...
#  ifdef IR_HITACHI_AC3
  if (strcmp(protocol_name, "HITACHI_AC3") == 0) {
    ...
    if (valueRPT == repeatIRwNumber)
      valueRPT = std::max(valueRPT, kHitachiAcDefaultRepeat);
    if (valueBITS == 0)
      Logger.error(0, F("For this protocol you should have a BIT number as there is no default one defined" CR));
...
#  endif
  Logger.warning(0, F("Unknown IR protocol" CR));
  return false;
}
#endif
