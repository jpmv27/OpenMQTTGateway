...
void errorDecoder(SHTC3_Status_TypeDef message) // The errorDecoder function prints "SHTC3_Status_TypeDef" resultsin a human-friendly way
{
  switch (message) {
    case SHTC3_Status_Nominal:
      Logger.notice(0, F("Nominal"));
      break;
    case SHTC3_Status_Error:
      Logger.error(0, F("Error"));
      break;
    case SHTC3_Status_CRC_Fail:
      Logger.error(0, F("CRC Fail"));
      break;
    default:
      Logger.error(0, F("Unknown return code"));
      break;
  }
}
...
void MeasureTempAndHum() {
  if (millis() > (timedht + TimeBetweenReadingSHTC3)) { //retrieving value of temperature and humidity of the box from SHTC3 every xUL
    ...
    if (mySHTC3.lastStatus == SHTC3_Status_Nominal) {
      ...
      if (isnan(h) || isnan(t)) {
        Logger.error(0, F("Failed to read from SHTC3 sensor!" CR));
      } else {
        Logger.debug(0, F("Creating SHTC3 buffer" CR));
