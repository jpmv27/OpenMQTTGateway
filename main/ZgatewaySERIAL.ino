...
#      endif
  SERIALStream = &Serial;
  Logger.notice(0, F("SERIAL HW UART0" CR));
...
#      endif
  ...
  Logger.notice(0, F("SERIAL HW UART1" CR));
...
#      endif
  ...
  Logger.notice(0, F("SERIAL HW UART2" CR));
...
#    elif SERIAL_UART == 3 // init UART3
  ...
  Logger.notice(0, F("SERIAL HW UART3" CR));
#    endif
...
#  else // Software serial
  ...
  Logger.notice(0, F("SERIAL_RX_GPIO: %d" CR), SERIAL_RX_GPIO);
  Logger.notice(0, F("SERIAL_TX_GPIO: %d" CR), SERIAL_TX_GPIO);
#  endif

#  ifdef ESP32
  serialSemaphore = xSemaphoreCreateMutex();
  if (serialSemaphore == NULL) {
    Logger.error(0, F("Failed to create serialSemaphore" CR));
  }
#  endif
  ...
  Logger.notice(0, F("SERIALBaud: %d" CR), SERIALBaud);
  Logger.debug(0, F("ZgatewaySERIAL setup done" CR));
}
...
void SERIALtoX() {
  ...
  if (SERIALStream->available()) {
    Logger.debug(0, F("SERIALtoMQTT" CR));
    ...
    Logger.notice(0, F("SERIAL data: %s" CR), output);
...
void sendHeartbeat() {
  if (SEMAPHORE_SERIAL) {
    ...
    SERIALStream->flush();
    Logger.notice(0, F("Sent Serial heartbeat" CR));
    SEMAPHORE_SERIAL_GIVE;
  } else {
    Logger.error(0, F("Failed to take serialSemaphore" CR));
  }
}
...
void sendHeartbeatAck() {
  if (SEMAPHORE_SERIAL) {
    ...
    SERIALStream->print(SERIALPost);
    SERIALStream->flush();
    Logger.notice(0, F("Sent heartbeat ack" CR));
    SEMAPHORE_SERIAL_GIVE;
  } else {
    Logger.error(0, F("Failed to take serialSemaphore" CR));
  }
...
#    ifdef SENDER_SERIAL_HEARTBEAT
  ...
  if (currentTime - lastHeartbeatAckReceivedCheck > heartbeatAckCheckInterval) {
    ...
    if (currentTime - lastHeartbeatAckReceived > heartbeatTimeout) {
      ...
      Logger.warning(0, F("No heartbeat ack received. Increasing interval to %lu ms" CR), heartbeatInterval);
      receiverReady = false;
    } else {
    ...
    if (buffer.startsWith(SERIALPre) && buffer.endsWith(SERIALPost)) {
      ...
      if (err == DeserializationError::Ok) {
        ...
        if (SERIALdata.containsKey("type") && strcmp(SERIALdata["type"], "heartbeat") == 0) {
          ...
          receiverReady = true;
          Logger.notice(0, F("Heartbeat ack received" CR));
        } else {
          // Process normal messages
          Logger.notice(0, F("SERIAL msg received: %s" CR), jsonString.c_str());
#    if jsonPublishing
...
#    endif
        ...
        Logger.error(0, F("Error in SERIALJSONtoMQTT, deserializeJson() returned %s" CR), err.c_str());
      }
      ...
      Logger.error(0, F("Buffer error, clearing buffer. Partial content: %s" CR), buffer.c_str());
      buffer = "";
    } else if (buffer.length() > JSON_MSG_BUFFER) {
      // If the buffer gets too large without finding a complete message, clear it
      Logger.error(0, F("Buffer overflow, clearing buffer. Partial content: %s" CR), buffer.c_str());
      buffer = "";
      isOverflow = true;
    }
  }
...
void sendMQTTfromNestedJson(JsonVariant obj, char* topic, int level, int maxLevel) {
  ...
  if (level < maxLevel && obj.is<JsonObject>()) {
    ...
    for (JsonPair pair : obj.as<JsonObject>()) {
      ...
      Logger.debug(0, F("level=%d, key='%s'" CR), level, pair.key().c_str());
      if (topicLength + 2 + strlen(key) <= mqtt_topic_max_size) {
        ...
        Logger.error(0, F("Nested key '%s' at level %d does not fit within max topic length of %d, skipping"),
    ...
    Logger.notice(0, F("level=%d, topic=%s, value: %s\n"), level, topic, output);
...
bool XtoSERIAL(const char* topicOri, JsonObject& SERIALdata) {
  ...
  if (SEMAPHORE_SERIAL) {
    if (receiverReady && (cmpToMainTopic(topicOri, subjectMQTTtoSERIAL) ||
      ...
      Logger.notice(0, F("[ OMG->SERIAL ] data sent: %s" CR), data.c_str());
      res = true;
      delay(100);
    }
    SEMAPHORE_SERIAL_GIVE;
