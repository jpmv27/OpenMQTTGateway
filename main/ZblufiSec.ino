...
void blufi_dh_negotiate_data_handler(uint8_t* data, int len, uint8_t** output_data, int* output_len, bool* need_free) {
  ...
  if (blufi_sec == NULL) {
    Logger.error(OMG_LOGID, F("BLUFI Security is not initialized"));
  ...
  switch (type) {
    ...
    case SEC_TYPE_DH_PARAM_DATA: {
      if (blufi_sec->dh_param == NULL) {
        Logger.error(OMG_LOGID, F("%s, blufi_sec->dh_param == NULL"), __func__);
      ...
      if (ret) {
        Logger.error(OMG_LOGID, F("%s read param failed %d"), __func__, ret);
      ...
      if (ret) {
        Logger.error(OMG_LOGID, F("%s make public failed %d"), __func__, ret);
      ...
      ret = mbedtls_dhm_calc_secret(&blufi_sec->dhm,
                                    ...
                                    myrand, NULL);
      if (ret) {
        Logger.error(OMG_LOGID, F("%s mbedtls_dhm_calc_secret failed %d"), __func__, ret);
        btc_blufi_report_error(ESP_BLUFI_DH_PARAM_ERROR);
        return;
      }
