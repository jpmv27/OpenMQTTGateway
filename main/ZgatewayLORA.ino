...
LORAdevice* getDeviceById(const char* id) {
  Logger.debug(OMG_LOGID, F("getDeviceById %s"), id);
...
void dumpLORADevices() {
  for (std::vector<LORAdevice*>::iterator it = LORAdevices.begin(); it != LORAdevices.end(); ++it) {
    LORAdevice* p = *it;
    Logger.debug(OMG_LOGID, F("uniqueId %s"), p->uniqueId);
    Logger.debug(OMG_LOGID, F("modelName %s"), p->modelName);
...
void createOrUpdateDeviceLORA(const char* id, const char* model, uint8_t flags) {
  if (xSemaphoreTake(semaphorecreateOrUpdateDeviceLORA, pdMS_TO_TICKS(30000)) == pdFALSE) {
    Logger.error(OMG_LOGID, F("[LORA] semaphorecreateOrUpdateDeviceLORA Semaphore NOT taken"));
    return;
  }

  LORAdevice* device = getDeviceById(id);
  if (device == &NO_LORA_DEVICE_FOUND) {
    Logger.debug(OMG_LOGID, F("add %s"), id);
    //new device
    device = new LORAdevice();
    if (strlcpy(device->uniqueId, id, uniqueIdSize) > uniqueIdSize) {
      Logger.warning(OMG_LOGID, F("[LORA] Device id %s exceeds available space"), id); // Remove from production release ?
    };
    if (strlcpy(device->modelName, model, modelNameSize) > modelNameSize) {
      Logger.warning(OMG_LOGID, F("[LORA] Device model %s exceeds available space"), id); // Remove from production release ?
    };
    device->isDisc = flags & device_flags_isDisc;
    LORAdevices.push_back(device);
    newLORADevices++;
  } else {
    Logger.debug(OMG_LOGID, F("update %s"), id);
...
void launchLORADiscovery(bool overrideDiscovery) {
  ...
  for (std::vector<LORAdevice*>::iterator it = localDevices.begin(); it != localDevices.end(); ++it) {
    ...
    Logger.debug(OMG_LOGID, F("Device id %s"), pdevice->uniqueId);
    ...
    if (overrideDiscovery || !isDiscovered(pdevice)) {
      ...
      for (int i = 0; i < numRows; i++) {
        if (strstr(pdevice->uniqueId, LORAparameters[i][0]) != 0) {
          ...
          Logger.debug(OMG_LOGID, F("idWoKey %s"), idWoKey.c_str());
      ...
      if (!pdevice->isDisc) {
        Logger.debug(OMG_LOGID, F("Device id %s was not discovered"), pdevice->uniqueId); // Remove from production release ?
      }
    } else {
      Logger.debug(OMG_LOGID, F("Device already discovered or that doesn't require discovery %s"), pdevice->uniqueId);
    }
  }
}
...
void LORAConfig_load() {
  ...
  if (preferences.isKey("LORAConfig")) {
    ...
    preferences.end();
    Logger.notice(OMG_LOGID, F("LORA Config loaded"));
    if (error) {
      Logger.error(OMG_LOGID, F("LORA Config deserialization failed: %s, buffer capacity: %u"), error.c_str(), jsonBuffer.capacity());
      return;
    }
    if (jsonBuffer.isNull()) {
      Logger.warning(OMG_LOGID, F("LORA Config is null"));
      return;
    }
    ...
    preferences.end();
    Logger.notice(OMG_LOGID, F("LORA Config not found"));
  }
}
...
void LORAConfig_fromJson(JsonObject& LORAdata) {
  ...
  if (LORAdata.containsKey("syncword")) {
    ...
    Logger.notice(OMG_LOGID, F("Config syncword changed: %d"), LORAConfig.syncWord);
  }
  ...
  if (LORAdata.containsKey("erase") && LORAdata["erase"].as<bool>()) {
    ...
    if (preferences.isKey("LORAConfig")) {
      ...
      Logger.notice(OMG_LOGID, F("LORA config erase result: %d"), result);
      preferences.end();
  ...
  if (LORAdata.containsKey("save") && LORAdata["save"].as<bool>()) {
    ...
    if (LORAConfig.syncWord < 0 || LORAConfig.syncWord > 255) {
      Logger.error(OMG_LOGID, F("Invalid syncWord value: %d"), LORAConfig.syncWord);
    } else {
      char syncWordHex[5];
    ...
    preferences.end();
    Logger.notice(OMG_LOGID, F("LORA Config_save: %s, result: %d"), conf.c_str(), result);
  }
}
...
#  ifdef OMG_MQTT_DISCOVERY
  ...
  xSemaphoreGive(semaphorecreateOrUpdateDeviceLORA);
#  endif
  Logger.notice(OMG_LOGID, F("LORA Frequency: %d"), LORAConfig.frequency);
#  ifdef ESP8266
  SPI.begin();
#  else
...
#  endif
  ...
  if (!LoRa.begin(LORAConfig.frequency)) {
    Logger.error(OMG_LOGID, F("ZgatewayLORA setup failed!"));
    while (1)
      ;
  }
  LoRa.receive();
  Logger.notice(OMG_LOGID, F("LORA_SCK: %d"), LORA_SCK);
  Logger.notice(OMG_LOGID, F("LORA_MISO: %d"), LORA_MISO);
  Logger.notice(OMG_LOGID, F("LORA_MOSI: %d"), LORA_MOSI);
  Logger.notice(OMG_LOGID, F("LORA_SS: %d"), LORA_SS);
  Logger.notice(OMG_LOGID, F("LORA_RST: %d"), LORA_RST);
  Logger.notice(OMG_LOGID, F("LORA_DI0: %d"), LORA_DI0);
  Logger.debug(OMG_LOGID, F("ZgatewayLORA setup done"));
}
...
void LORAtoX() {
  ...
  if (packetSize) {
    ...
    Logger.debug(OMG_LOGID, F("Rcv. LORA"));
#  ifdef ESP32
...
#  endif
    ...
    if (deviceId == WIPHONE) {
      ...
      if (LORAConfig.onlyKnown) {
        Logger.debug(OMG_LOGID, F("Ignoring non identifiable packet"));
        return;
      }
      ...
      if (result) {
        Logger.notice(OMG_LOGID, F("LORA packet deserialization failed, not a json, sending raw message"));
        LORAdata = LORAdataBuffer.to<JsonObject>();
        LORAdata["message"] = (char*)packet;
      } else {
        Logger.debug(OMG_LOGID, F("LORA packet deserialization OK"));
      }
    }
...
#  endif
    ...
    if (repeatLORAwMQTT) {
      Logger.debug(OMG_LOGID, F("Pub LORA for rpt"));
...
void XtoLORA(const char* topicOri, JsonObject& LORAdata) { // json object decoding
  if (cmpToMainTopic(topicOri, subjectMQTTtoLORA)) {
    ...
    if (message || hex) {
      ...
      LoRa.endPacket();
      Logger.debug(OMG_LOGID, F("MQTTtoLORA OK"));
      ...
      Logger.error(OMG_LOGID, F("MQTTtoLORA Fail json"));
    }
  }
  if (cmpToMainTopic(topicOri, subjectMQTTtoLORAset)) {
    Logger.debug(OMG_LOGID, F("MQTTtoLORA json set"));
    /*
...
void XtoLORA(const char* topicOri, const char* LORAarray) { // json object decoding
  if (cmpToMainTopic(topicOri, subjectMQTTtoLORA)) {
    ...
    LoRa.endPacket();
    Logger.notice(OMG_LOGID, F("MQTTtoLORA OK"));
    // we acknowledge the sending by publishing the value to an acknowledgement topic, for the moment even if it is a signal repetition we acknowledge also
