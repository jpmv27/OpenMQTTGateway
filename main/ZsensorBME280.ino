...
void setupZsensorBME280() {
  ...
  Logger.notice(0, F("Setup BME280/BMP280 on address: %X" CR), BME280_i2c_addr);
  //***Operation settings*****************************//
  ...
  if (ret == 0x60) {
    Logger.notice(0, F("Bosch BME280 successfully initialized: %X" CR), ret);
  } else if (ret == 0x58) {
    Logger.notice(0, F("Bosch BMP280 successfully initialized: %X" CR), ret);
  } else {
    Logger.notice(0, F("Bosch BME280/BMP280 failed: %X" CR), ret);
  }
}
...
void MeasureTempHumAndPressure() {
  if (millis() > (timebme280 + TimeBetweenReadingbme280)) {
    ...
    if (isnan(BmeTempC) || isnan(BmeTempF) || isnan(BmeHum) || isnan(BmePa) || isnan(BmeAltiM) || isnan(BmeAltiFt)) {
      Logger.error(0, F("Failed to read from BME280/BMP280!" CR));
    } else {
      Logger.debug(0, F("Creating BME280/BMP280 buffer" CR));
      ...
      if (BmeTempC != persisted_bme_tempc || bme280_always) {
        ...
        Logger.debug(0, F("Same Degrees C don't send it" CR));
      }
      ...
      if (BmeTempF != persisted_bme_tempf || bme280_always) {
        ...
        Logger.debug(0, F("Same Degrees F don't send it" CR));
      }
      ...
      if (BmeHum != persisted_bme_hum || bme280_always) {
        ...
        Logger.debug(0, F("Same Humidity don't send it" CR));
      }
      ...
      if (BmePa != persisted_bme_pa || bme280_always) {
        ...
        Logger.debug(0, F("Same Pressure don't send it" CR));
      }

      // Generate Altitude in Meter
      if (BmeAltiM != persisted_bme_altim || bme280_always) {
        Logger.debug(0, F("Sending Altitude Meter to MQTT" CR));
        BME280data["altim"] = (float)BmeAltiM;
      } else {
        Logger.debug(0, F("Same Altitude Meter don't send it" CR));
      }
      ...
      if (BmeAltiFt != persisted_bme_altift || bme280_always) {
        ...
        Logger.debug(0, F("Same Altitude Feet don't send it" CR));
      }
