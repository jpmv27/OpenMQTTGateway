...
void setupHCSR04() {
  Logger.notice(0, F("HCSR04 trigger pin: %d" CR), HCSR04_TRI_GPIO);
  Logger.notice(0, F("HCSR04 echo pin: %d" CR), HCSR04_ECH_GPIO);
...
void MeasureDistance() {
  if (millis() > (timeHCSR04 + TimeBetweenReadingHCSR04)) {
    timeHCSR04 = millis();
    Logger.debug(0, F("Creating HCSR04 buffer" CR));
    ...
    if (isnan(duration)) {
      Logger.error(0, F("Failed to read from HC SR04 sensor!" CR));
    } else {
      ...
      if (d > distance) {
        ...
        Logger.debug(0, F("HC SR04 Distance changed" CR));
      } else if (d < distance) {
        HCSR04data["direction"] = "towards";
        Logger.debug(0, F("HC SR04 Distance changed" CR));
      } else if (HCSR04_always) {
        HCSR04data["direction"] = "static";
        Logger.debug(0, F("HC SR04 Distance hasn't changed" CR));
      }
      distance = d;
