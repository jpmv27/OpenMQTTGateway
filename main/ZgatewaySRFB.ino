...
void setupSRFB() {
  Logger.debug(OMG_LOGID, F("ZgatewaySRFB setup done "));
  Logger.debug(OMG_LOGID, F("Serial Baud: %l"), SERIAL_BAUD);
}
...
void _rfbSend(byte* message, int times) {
  ...
  Logger.notice(OMG_LOGID, F("[RFBRIDGE] Sending MESSAGE"));
...
void _rfbDecode() {
  ...
  if (action == RF_CODE_RFIN) {
    ...
    Logger.debug(OMG_LOGID, F("Creating SRFB buffer"));
    ...
    if (!isAduplicateSignal(MQTTvalue) && MQTTvalue != 0) { // conditions to avoid duplications of RF -->MQTT
      Logger.debug(OMG_LOGID, F("Adv data SRFBtoMQTT"));
      SRFBdata["origin"] = subjectSRFBtoMQTT;
      enqueueJsonObject(SRFBdata);
      Logger.debug(OMG_LOGID, F("Store val: %lu"), MQTTvalue);
      storeSignalValue(MQTTvalue);
      if (repeatSRFBwMQTT) {
        Logger.debug(OMG_LOGID, F("Publish SRFB for rpt"));
...
void XtoSRFB(const char* topicOri, const char* datacallback) {
  ...
  if (topic == subjectMQTTtoSRFB) {
    ...
    if (pos != -1) {
      ...
      Logger.notice(OMG_LOGID, F("SRFB Repeat: %d"), valueRPT);
    }
    ...
    if (pos2 != -1) {
      ...
      Logger.notice(OMG_LOGID, F("RF Mini Pulse Lgth: %d"), valueMiniPLSL);
    }
    ...
    if (pos3 != -1) {
      ...
      Logger.notice(OMG_LOGID, F("RF Maxi Pulse Lgth: %d"), valueMaxiPLSL);
    }
    ...
    if (pos4 != -1) {
      ...
      Logger.notice(OMG_LOGID, F("RF sync: %d"), valueSYNC);
    }

    Logger.debug(OMG_LOGID, F("MQTTtoSRFB prts"));
    if (valueRPT == 0)
      valueRPT = 1;
    if (valueMiniPLSL == 0)
      valueMiniPLSL = 320;
...
void XtoSRFB(const char* topicOri, JsonObject& SRFBdata) {
  ...
  if (cmpToMainTopic(topicOri, subjectMQTTtoSRFB)) {
    Logger.debug(OMG_LOGID, F("MQTTtoSRFB json"));
    if (raw) { // send raw in priority when defined in the json
      Logger.debug(OMG_LOGID, F("MQTTtoSRFB raw ok"));
      byte message_b[RF_MESSAGE_SIZE];
      ...
      if (data != 0) {
        Logger.notice(OMG_LOGID, F("MQTTtoSRFB data ok"));
        ...
        Logger.notice(OMG_LOGID, F("MQTTtoSRFB OK"));
        _rfbSend(message_b, valueRPT);
        ...
        Logger.error(OMG_LOGID, F("MQTTtoSRFB error decoding value"));
      }
    }
  }
}
