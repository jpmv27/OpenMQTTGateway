...
void setupZsensorDS1820() {
  Logger.debug(OMG_LOGID, F("DS1820: configured pin: %d for 1-wire bus"), DS1820_OWBUS_GPIO);
  ds1820.begin();
  ...
  Logger.notice(OMG_LOGID, F("DS1820: Found %d devices"), numDevicesOnBus);
  ...
  for (int deviceIndex = 0; deviceIndex < numDevicesOnBus && ds1820_count < OW_MAX_SENSORS; deviceIndex++) {
    ...
    if (ds1820.getAddress(ds1820_address, deviceIndex) && ds1820.validFamily(ds1820_address)) {
      ...
      Logger.debug(OMG_LOGID, F("DS1820: Device %d, Type: %s, Address: %s, Resolution: %d"),
                ds1820_count,
  ...
  if (ds1820.getDS18Count() == 0) {
    Logger.error(OMG_LOGID, F("DS1820: Failed to enumerate sensors on 1-wire bus. Check your GPIO assignment!"));
  }
...
void MeasureDS1820Temp() {
  ...
  if (!triggeredConversion && ((millis() - timeDS1820) > (DS1820_INTERVAL_SEC * 1000UL - DS1820_CONV_TIME))) {
    Logger.debug(OMG_LOGID, F("DS1820: Trigger temperature conversion..."));
    ...
    if (ds1820_count < 1) {
      Logger.error(OMG_LOGID, F("DS1820: Failed to identify any temperature sensors on 1-wire bus during setup!"));
    } else {
      Logger.debug(OMG_LOGID, F("DS1820: Reading temperature(s) from %d sensor(s)..."), ds1820_count);
      ...
      for (uint8_t i = 0; i < ds1820_count; i++) {
        ...
        if (current_temp[i] == -127) {
          Logger.error(OMG_LOGID, F("DS1820: Device %s currently disconnected!"), (char*)ds1820_addr[i].c_str());
          ...
          Logger.debug(OMG_LOGID, F("DS1820: Temperature for device %s didn't change, don't publish it."), (char*)ds1820_addr[i].c_str());
        }
